  ...
  public async createOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!userId) {
        res.status(401).json({
          message:
            "Không tìm thấy thông tin người dùng. Vui lòng đăng nhập để tiếp tục.",
        });
        return;
      }
      const ip = req.headers["x-forwarded-for"]?.toString() || "127.0.0.1";
      const orderData = {
        ...req.body,
        userId: userId,
        ip: ip,
      ...
      res.status(500).json({
        message:
          "Đã có lỗi Server xảy ra khi tạo đơn hàng. Vui lòng thử lại sau.",
      });
    }
  }

  public getMyOrdersService = async (
    req: Request,
    res: Response,
  ): Promise<void> => {
    try {
      ...
      res.status(500).json({
        message:
          "Đã có lỗi Server xảy ra khi lấy đơn hàng của người dùng. Vui lòng thử lại sau.",
      });
    }
  };
}
