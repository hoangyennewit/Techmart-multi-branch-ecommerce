...
export class OrderService {
    ...
    public createOrder = async (orderData: any) => {
        ...
        try {
            ...
            const values = items.map((_: any, i: number) => `($1, $${i * 3 + 2}, $${i * 3 + 3}, $${i * 3 + 4})`).join(',');
            const flatItems = items.reduce((acc: any, item: any) => [...acc, item.productId, item.quantity, item.price], [newOrderId]);
            await sequelize.query(
            ...
            if(paymentMethod === 'vnpay') {
                return await this.vnpayService.createdPaymentUrl(newOrderId, tongTien);
            }
            else {
                return await this.codService.processPayment(newOrderId);
            }
        }
        catch (error) {
            await t.rollback();
