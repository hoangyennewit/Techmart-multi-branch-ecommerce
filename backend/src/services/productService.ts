...
export class ProductService {
  ...
  public searchProducts = async (searchTerm: string) => {
    try {
      if (!searchTerm || searchTerm.trim().length === 0) {
        return [];
      }

      // Tìm sản phẩm có tên bắt đầu bằng keyword
      const startPattern = `${searchTerm.trim()}%`;
      const products: any = await sequelize.query(
        `SELECT
                    ma_san_pham AS id,
                    ma_danh_muc AS categoryId,
                ...
                FROM san_pham
                WHERE trang_thai = 1 
                    AND LOWER(ten_san_pham) LIKE LOWER($1)
