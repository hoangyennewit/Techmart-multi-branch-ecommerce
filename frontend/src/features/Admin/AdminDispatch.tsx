...
export const AdminDispatch = () => {
  ...
  const handleApprove = async (id: number) => {
    ...
    } catch (error) { /* Bỏ qua lỗi để test UI */ }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
          ...
          <button className="px-6 py-2 bg-purple-700 text-white font-medium rounded-full shadow-md relative whitespace-nowrap">
            Phê duyệt điều phối
            {pendingCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full">{pendingCount}</span>
            )}
          </button>
