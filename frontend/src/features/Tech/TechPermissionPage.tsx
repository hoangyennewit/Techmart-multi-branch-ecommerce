...
export const TechPermissionPage = () => {
  // 1. Danh sách các chức vụ (Dựa theo ảnh 1)
  ...
  // 2. State quản lý Modal (Hình 2)
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newRoleName, setNewRoleName] = useState('');

  // 3. Logic Bật/Tắt quyền
  ...
  const handleSaveRole = () => {
    ...
    setRoles([...roles, newRole]); // Thêm vào cuối danh sách
    setIsModalOpen(false); // Đóng popup
    setNewRoleName(''); // Xóa text đã gõ
  };
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
          <div className="flex gap-3">
            {/* Tab Tài khoản (Mờ - Bấm vào sẽ dùng Router chuyển file sau) */}
          ...
          {/* NÚT THÊM CHỨC VỤ MỞ POPUP */}
          <button 
            onClick={() => setIsModalOpen(true)}
            className="px-8 py-3 bg-gray-800 text-white font-bold rounded-full shadow-lg hover:bg-purple-700 transition-all hover:-translate-y-0.5"
          >
            + Thêm chức vụ
          </button>
        </div>

        {/* --- GRID HIỂN THỊ CÁC THẺ PHÂN QUYỀN --- */}
        ...
        {/* --- POPUP (MODAL) THÊM CHỨC VỤ (HÌNH 2) --- */}
        {isModalOpen && (
          <div className="absolute top-24 right-0 z-50 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-white p-6 rounded-[2rem] shadow-2xl border border-gray-100 w-[380px] relative">
              ...
              <div className="flex items-center gap-3 mb-6 relative z-10">
                <label className="text-gray-500 font-bold text-sm whitespace-nowrap">
                  Tên chức vụ:
                </label>
                <input 
                  type="text" 
                  ...
                  className="flex-1 bg-gray-50 border border-gray-200 text-gray-800 rounded-full px-4 py-2 outline-none focus:bg-white focus:border-purple-400 focus:ring-1 focus:ring-purple-400 text-sm font-medium transition-all"
                  autoFocus
                />
              </div>
              ...
              <div className="flex gap-3 justify-end relative z-10">
                <button 
                  ...
                  className="px-8 py-2.5 bg-red-500 hover:bg-red-600 text-white font-bold rounded-full text-sm transition-colors shadow-sm"
                >
                  Hủy
                </button>
                <button 
                  onClick={handleSaveRole} 
                  className="px-8 py-2.5 bg-emerald-400 hover:bg-emerald-500 text-white font-bold rounded-full text-sm transition-colors shadow-sm"
                >
                  Lưu
                </button>
              </div>
