...
export const OrderTrackingPage = () => {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            case "processing":
                return { label: "Đang xử lý", color: "text-blue-500", bg: "bg-blue-500", icon: Package, step: 2 };
            case "shipping":
                return { label: "Đang giao hàng", color: "text-purple-500", bg: "bg-purple-500", icon: Truck, step: 3 };
            case "delivered":
                return { label: "Đã giao hàng", color: "text-green-500", bg: "bg-green-500", icon: CheckCircle, step: 4 };
            default: // pending
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-16">
            ...
            <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                <h1 className="text-2xl font-bold text-gray-800 mb-8">Theo dõi Đơn hàng</h1>

                {orders.length === 0 ? (
                    <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
                        <Package className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                        <h2 className="text-xl font-bold text-gray-800 mb-2">Bạn chưa có đơn hàng nào</h2>
                        <p className="text-gray-500 mb-6">Hãy tham quan các mặt hàng công nghệ mới nhất tại TechMart nhé!</p>
                        <Link to="/" className="inline-block bg-orange-500 text-white font-bold py-3 px-8 rounded-full shadow-lg shadow-orange-200 hover:bg-orange-600 transition-colors">
                            Bắt đầu mua sắm
                        </Link>
                    </div>
                ) : (
                                ...
                                <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100 mb-6">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="font-black text-orange-500 text-lg">{formatPrice(order.totalAmount + order.shippingFee)}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative mb-8 px-4">
                                        ...
                                        <div className="absolute top-1/2 left-0 h-1 bg-orange-500 -translate-y-1/2 rounded-full z-0 transition-all duration-500"
                                             style={{ width: `${(statusInfo.step - 1) * 33.33}%` }}></div>
                                        ...
                                        <div className="relative z-10 flex justify-between">
                                            {[
                                                ...
                                                <div key={s.step} className="flex flex-col items-center gap-2">
                                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors border-4 
                                                        ${statusInfo.step >= s.step 
                                                            ? "bg-orange-500 border-orange-100 text-white" 
                                                            : "bg-white border-gray-100 text-gray-400"}`}
                                                    >
                                                        {statusInfo.step > s.step ? <CheckCircle className="w-4 h-4" /> : s.step}
                                                    </div>
                                                    <span className={`text-xs font-semibold hidden sm:block ${statusInfo.step >= s.step ? "text-gray-800" : "text-gray-400"}`}>
                                                        {s.label}
                                                    </span>
                                                </div>
                                            ))}
